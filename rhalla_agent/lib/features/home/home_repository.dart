...
/// حوالاتٌ **خارجية** أرسلها الوكيل ولم تُعتمد بعد.
///
/// ══════════════════════════════════════════════════════════════════════════
///  أمرُ المالك (11 سبتمبر 2026): تظهر الخارجيةُ ضمن «الصادرة»
/// ══════════════════════════════════════════════════════════════════════════
///
/// ⚠ **والمعتمدةُ منها كانت تظهر أصلاً**: كشفُ الحساب يحملها باسم «حوالة
/// خارجية صادرة»، وتبويبُ «صادرة» يقرأ الكشف. الغائبُ هو ما لم يُعتمد بعد —
/// والقيدُ المحاسبيّ لا يُكتب إلّا عند الاعتماد في المكتب الخلفي، وهو **مسارُ
/// المنظومة ولم يُغيَّر منه حرف** (أكّده المالك صراحةً).
///
/// فهذه قراءةٌ من `ExternalEx` تُظهر الحوالة موسومةً «بانتظار الاعتماد»، لا
/// قيدٌ يُكتب قبل أوانه: الوكيل يعرف أنّ حوالته وقعت وأين هي، والمحاسبةُ
/// تبقى حيث هي.
///
/// وهي نظيرةُ [pendingOutgoingProvider] بدفترها هي — والخادم يشكّلها بمفاتيح
/// كشف الحساب نفسِها، فتُقرأ بـ[Movement] وتُعرض ببطاقته: لا نموذجَ ثانٍ ولا
/// بطاقةَ ثانية.
final externalOutgoingProvider =
    FutureProvider.autoDispose<List<Movement>>((ref) async {
  final api = ref.watch(apiClientProvider);
  try {
    final env = await api.get('/agent/outgoing-transfers/external');
    return env.rows.map(Movement.fromJson).toList();
  } on ApiFailure catch (e) {
    if (e.isEmptyResult) return const [];
    rethrow;
