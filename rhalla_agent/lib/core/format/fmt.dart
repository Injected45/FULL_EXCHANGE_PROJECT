...
class Fmt {
  ...
  /// الساعة والدقيقة وحدهما: «14:33».
  ///
  /// للمحادثات (قرار المالك، 5 سبتمبر 2026): التاريخ يُكتب مرّةً في فاصل
  /// اليوم أعلى مجموعة الرسائل، لا على كل فقاعة — وتكرارُه على كل سطر ضجيجٌ
  /// يزاحم النصّ. والثواني لا يقرؤها أحد في محادثة.
  ///
  /// وما لا وقت فيه يعود فارغاً لا تاريخاً: هذه الدالّة تَعِد بساعة.
  static String hhmm(String? raw) {
    final s = (raw ?? '').trim();
    if (s.isEmpty) return '';
    final m = RegExp(r'(\d{1,2}):(\d{2})').firstMatch(s);
    if (m == null) return '';
    return '${m[1]!.padLeft(2, '0')}:${m[2]}';
  }
