...
void main() {
  ...
  testWidgets('⚠ كلُّ مفتاحٍ وحدَه: لا لافتةَ مع بلاطة، ولا صمتَ بلا سبب',
     ...
     * الموظف لا عملَه — فصار العددُ اثنين بلا بلاطةٍ واحدة، وفشل الفحصُ على
     * `CLOSE_SHIFT` مُبلّغاً عن تناقضٍ لا وجود له.
     *
     * فيُقاس أثاثُ الشاشة الثابت بتركيبها **بلا صلاحية واحدة**: ما زاد عليه
     * بلاطة. وزرٌّ يُضاف إلى الرأس غداً يدخل في القياس من تلقاء نفسه، بينما
     * تبقى البلاطةُ الحقيقية مكشوفةً كما كانت.
     */
    await _mount(tester, const []);
    final chrome = _tiles().evaluate().length;

    // ⚠ وخطُّ الأساس نفسُه محدود: لو صار يبني بلاطاتٍ بلا صلاحية لابتلعها
    // القياسُ صامتاً وصار الفحصُ يمرّ على العيب الذي كُتب له. أربعةٌ سقفٌ
    // لأثاث الرأس (خروج · أمان · وما قد يُضاف)، وتجاوزُه يستحقّ نظرة.
    expect(chrome, lessThan(4),
    ...
    for (final key in catalog) {
      ...
      final hasTile = _tiles().evaluate().length > chrome;
