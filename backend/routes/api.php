...
/* ══════════════════════════════════════════════════════════════════════════
   رمزُ تأكيد الحوالة — إلى هاتف الموظف المعتمد (11 سبتمبر 2026)
   ══════════════════════════════════════════════════════════════════════════

   أمرُ المالك: «كلُّ موظفٍ يتعامل برمز التحقق الذي يصله على هاتفه … الموظفُ
   لا ينتظر الوكيل». والرقمُ هو الذي اعتمده الوكيلُ عند التفعيل — وبه فُتح
   التطبيق أصلاً.

   ⚠ **بلا صلاحيةٍ إضافية**: هذه خطوةُ تأكيدٍ داخل عملٍ مأذونٍ به أصلاً
   (`CREATE_TRANSFER` أو `CREATE_EXTERNAL_TRANSFER`)، ومفتاحٌ ثالثٌ لها يعني
   وكيلاً يمنح الإنشاءَ ويظنّ أنه أتمّه بينما التأكيدُ مغلق.

   ⚠ **والخنقُ كخنق رمز الوكيل**: كلُّ نداءٍ يرسل واتساب حقيقياً ويحذف رموزَ
   الهاتف السابقة. */
Route::post('device/employee/otp/send',
    [ EmployeeController::class , 'sendTransferOtp' ])
    ->middleware(['employee', 'throttle:5,1']);

Route::post('device/employee/otp/verify',
    [ EmployeeController::class , 'verifyTransferOtp' ])
    ->middleware(['employee', 'throttle:10,1']);
