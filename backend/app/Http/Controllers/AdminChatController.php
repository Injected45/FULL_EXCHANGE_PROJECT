...
/**
 * ⚠⚠ **متروكةٌ — استُبدلت بمركز «الرحالة للدعم الفني» على `/support`.** ⚠⚠
 *
 * ما لم يكن يستطيعه هذا الملفّ وصار متاحاً هناك: حسابٌ لكلّ موظّف بدل مفتاحٍ
 * مشترك، وأدوارٌ وصلاحياتٌ مرفوضةٌ افتراضياً، وإسنادُ المحادثات وحالاتُها،
 * وسجلُّ نشاطٍ يقول من فعل ماذا.
 *
 * وبقاؤها حيّةً **يُبطل ذلك كلَّه**: من يعرف المفتاح يقرأ محادثات الوكلاء
 * جميعاً ويردّ باسمِ من شاء، بلا دورٍ ولا صلاحيةٍ ولا أثرٍ في السجلّ. فهي
 * بابٌ خلفيّ حول النظام الجديد لا صفحةٌ قديمة.
 *
 * **لإغلاقها: أفرغ `CHAT_ADMIN_KEY` في `.env`** ثم `php artisan config:clear`.
 * عندها تعود `key()` بـ `null` ويُرفض كلُّ دخول. ولم تُحذف الشيفرة لأن
 * إفراغَ المفتاح قرارُ المالك لا قرارُ الشيفرة، وحذفُها قبله يترك الإدارة
