...
class EmployeeController extends BaseController
{
    ...
    /*
     * ══════════════════════════════════════════════════════════════════════
     *  رمزُ تأكيد الحوالة — إلى هاتف **الموظف** نفسِه
     * ══════════════════════════════════════════════════════════════════════
     *
     * أمرُ المالك (11 سبتمبر 2026): «حوالةٌ داخلية أو خارجية مُنفَّذة من تطبيق
     * الموظف يُرسَل رمزُ التحقق على رقم الواتس المرخَّص به الوكيلُ لذلك
     * الموظف — لأنّ كلّ موظفٍ له رقمٌ مخصَّصٌ معتمدٌ من الوكيل، وإلّا كيف فتح
     * التطبيق أصلاً؟ … **الموظفُ لا ينتظر الوكيل**».
     *
     * ── وهو ما يجعل هذا التوحيد ممكناً أصلاً ────────────────────────────
     *
     * كان الموظف مُستثنى من الرمز بقرارٍ مسجَّل، وسببُه أنّ رمزَ شاشة المراجعة
     * يُرسَل إلى **هاتف الوكيل** — وهو غائبٌ حين يقف الموظف خلف الشبّاك،
     * فاشتراطُه كان يُبطل الميزة لا يحرسها. والتوثيقُ يومَها قال: «ولو أُريد
     * للموظف رمزٌ بالقوّة نفسِها فمحلُّه **هاتفُ الموظف** لا هاتفُ الوكيل —
     * وهاتفُه موثَّقٌ عند التفعيل، فالبنيةُ قائمة».
     *
     * وهذا هو ما نُفِّذ حرفياً.
     *
     * ⚠ **والهاتفُ من الجلسة لا من الطلب.** قراءتُه من جسم الطلب تعني موظفاً
     * يوجّه رمزَ تأكيد حوالته إلى أيّ هاتفٍ يكتبه — فيسقط الحارسُ كلُّه وهو
     * يبدو قائماً. وهو الشكلُ نفسُه الذي أُغلق في `transInsert` و`addUserTrans`
     * و`transInsertExternal`.
     *
     * ⚠ **ولا منطقَ رموزٍ ثانٍ.** يُنادى `OtpController` نفسُه الذي يخدم دخولَ
     * الوكيل: المهلةُ والتوليدُ والاستهلاكُ وقناةُ الواتساب واحدة. ونسخةٌ ثانية
     * كانت ستفترق عن الأولى عند أوّل تعديلٍ في المهلة أو الطول.
     */

    /** POST employee/otp/send — رمزُ تأكيدٍ إلى هاتف الموظف المعتمد. */
    public function sendTransferOtp(Request $request)
    {
        [$employee, , ] = $this->ctx($request);

        $phone = preg_replace('/[^0-9]/', '', (string) ($employee->phone ?? ''));

        if ($phone === '') {
            return $this->sendError(
                'لا رقم هاتفٍ مسجَّلٌ لك — راجع وكيلك.', [], 422);
        }

        // ⚠ يُدهَس ما في الطلب: الرقمُ من القاعدة وحدَه.
        $request->merge(['phone' => $phone]);

        return app(OtpController::class)->sendOtp($request);
    }

    /** POST employee/otp/verify — تحقّقٌ من الرمز على هاتف الموظف. */
    public function verifyTransferOtp(Request $request)
    {
        ...
        return app(OtpController::class)->checkOtp($request);
    }
