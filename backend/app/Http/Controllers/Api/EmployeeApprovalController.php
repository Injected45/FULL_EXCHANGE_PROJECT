...
class EmployeeApprovalController extends BaseController
{
    ...
    private function decide(Request $request, int $id, bool $approve)
    {
        ...
        /*
         * ══════════════════════════════════════════════════════════
         *  ⚠ موافقةُ الوكيل **إذنٌ لا تنفيذ**
         * ══════════════════════════════════════════════════════════
         *
         * أمرُ المالك (8 سبتمبر 2026): «يفترض لمّا يوافق الوكيل ترجع
         * لتُنفَّذ من قِبل الموظف، لأن المال مع الموظف وتدخل ضمن عهدته
         * وخزينته وباسمه».
         *
         * وهذا يصحّح ما كان: الوكيل يوافق فتُنفَّذ الحوالةُ في اللحظة
         * نفسِها من خادمه. والنقدُ حينئذٍ لم يُسلَّم بعد — الزبونُ واقفٌ
         * عند الموظف، والمالُ في يده هو. فحوالةٌ تُنفَّذ قبل أن يقبض
         * الموظفُ المال تُدخل خزينتَه في عجزٍ عن حوالةٍ لم يستلم قيمتها.
         *
         * فالموافقةُ تُثبَّت وتقف هنا. والتنفيذُ فعلُ الموظف، بيده،
         * وباسمه، ومع حركة خزينته — انظر
         * `EmployeeController::executeApproval`.
         */
        return $this->sendResponse([
            'request'  => EmployeeApprovals::present($this->approvals->find($id)),
            'executed' => false,
        ], 'تمت الموافقة. ينفّذ الموظف الحوالة الآن من تطبيقه.');
    }
