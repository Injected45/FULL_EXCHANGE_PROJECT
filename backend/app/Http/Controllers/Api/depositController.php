...
", [$request->country_id]);  // Replace $country_id with your actual variable
  ...
  public function LocalStatmentAccount(Request $request)
  {
      ...
      /* ربط العمولة بحوالتها — بند 11 و12 من مستند المالك.
       *
       * العلاقة **قائمة أصلاً في البيانات** ولم تُنشأ هنا: صفّ «عمولة تحويل»
       * يحمل `ISID` الحوالة الأمّ نفسه. تحقّقتُ منه في القاعدة: الحوالة
       * 13151-54-1 وعمولتها يتشاركان الرقم. فلا جدول جديد ولا مفتاح أجنبي
       * ولا هجرة — البند 12 يوجب استعمال القائم لا تكرارَه.
       *
       * والربط **مرجعيّ صريح**: بالرقم لا بترتيب الصفوف ولا بالتاريخ ولا
       * بالقيمة — وهي كلّها ممنوعة صراحةً في البند 11.
       *
       * ⚠ ولا يمسّ هذا شيئاً محاسبياً: صفّ العمولة يبقى قيداً مستقلاً في
       * `EX24AccSafeActivityTb` كما هو، ويبقى في الرد. المتغيّر أن التطبيق
       * صار يعرفه تابعاً فيعرضه داخل بطاقة حوالته بدل بطاقة مستقلّة.
       *
       * ولا استعلامٌ إضافي أصلاً (بند 14): الجمع يقع في PHP على الصفوف
       * المقروءة نفسها. كشف الحساب يعيد **كل** حركات الوكيل، وصفوف العمولة
       * منها — فما يلزم لربط العمولة بحوالتها موجودٌ في اليد قبل أن يُسأل
       * عنه الخادم.
       *
       * ولم يكن هذا اختياراً جمالياً بل قياساً. جُرّبت صيغتان قبله على
       * البيانات نفسها (6 صفوف لحساب واحد):
       *
       *   - جدولٌ مشتقّ مجمَّع (`leftJoinSub`): المخطِّط يقدّر تجميعاً على
       *     `EX24AccSafeActivityTb` كاملاً فيطلب منحة ذاكرة ضخمة، فينتظر
       *     الاستعلام على `RESOURCE_SEMAPHORE` أكثر من 70 ثانية ثم يسقط
       *     الطلب بـ «Maximum execution time exceeded».
       *   - استعلامٌ فرعيّ مرتبط: صحيحٌ في نتيجته، و**68 ثانية** — لا فهرس
       *     على `(accidfrom, ISID)` في جدولٍ يتجاوز نصف مليون صفّ، فكل صفّ
       *     يمسحه كاملاً.
       *
       * وإضافة فهرس إلى جدول مالي ممنوعة بالأمر المستدام، فبقي الجمع في
       * التطبيق: صفرُ استعلامات، ومرورٌ واحد على صفوفٍ في اليد.
       */
      ...
      ->selectRaw("
          ...
          -- هل هذا الصفّ عمولة؟ يُحسم في الخادم لا في التطبيق: تعريفٌ واحد
       ...
       *
       * كل صفوف الوكيل هنا سلفاً، وكلها لحسابه هو (`b.id = $userId` يربط
       * `a.accidfrom` بـ `AccID` واحد)، فالتجميع على `ISID` وحده يطابق
       * حرفياً ما كان يفعله التجميع على (accidfrom, ISID).
       *
       * والحقل يبقى رقماً دائماً كما كان `ISNULL(...,0)` يعيده — التمييز الذي
       * يشترطه البند 22 بين «بلا عمولة» و«لم تصل» يقوم في التطبيق على
       * **وجود المفتاح** لا على قيمته، فإرسال `null` كان سيخترع حالةً ثالثة
       * لم تُختبَر.
       */
      $commissionByCode = [];
      foreach ($results as $row) {
          if ((int) $row->IsCommission !== 1 || $row->Code === null || $row->Code === '') {
              continue;
          }
          $commissionByCode[$row->Code] = ($commissionByCode[$row->Code] ?? 0) + (float) $row->RawValue;
      }

      foreach ($results as $row) {
          $row->CommissionAmount = $commissionByCode[$row->Code] ?? 0;
          unset($row->RawValue);
      }
