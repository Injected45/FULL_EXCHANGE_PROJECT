...
class EmployeeAdminController extends BaseController
{
    ...
    public function setPermissions(Request $request, int $id)
    {
        ...
        /*
         * ══════════════════════════════════════════════════════════════════
         *  ⚠⚠ مفتاحٌ **مُتقاعِد** يُسقَط صامتاً؛ ومفتاحٌ **ممنوعٌ** يُرَدّ صراحةً
         * ══════════════════════════════════════════════════════════════════
         *
         * بلاغُ المالك (10 سبتمبر 2026): منح صلاحيةً موسومةً «تُمنح يدوياً»
         * فرُدَّ عليه بـ«صلاحية غير معروفة أو لا تُمنح لموظف»، ولم يُحفظ شيء.
         *
         * ── ولم تكن المشكلةُ في الصلاحية التي منحها ──────────────────────
         *
         * الشاشةُ تُرسل **مجموعة الممنوح كاملةً** لا ما تغيّر. وموظفوه يحملون
         * صفوفَ صلاحياتٍ أُلغيت مع نظام الوردية والعهدة (`VIEW_OWN_CASHBOX`
         * و`START_SHIFT` و…) — تُركت في القاعدة عمداً لأنها خاملةٌ ولا مسارَ
         * يفرضها، وحذفُها يمحو أثرَ من مُنح ماذا. فكانت كلُّ عملية حفظٍ تُرسلها
         * معها، ويسقط الحفظُ كلُّه على أوّل مفتاحٍ لم يعد في الكتالوج — أيّاً
         * كان ما أراد الوكيل تغييره.
         *
         * أي أنّ **إلغاء ميزةٍ كان يُجمّد شاشةَ الصلاحيات لكل موظفٍ كان يملكها**.
         *
         * ── والتفريقُ بين حالتين كان مطموساً في شرطٍ واحد ─────────────────
         *
         * • **ليس في الكتالوج** ⇦ مفتاحٌ تقاعد (أو خطأٌ مطبعيّ من عميلٍ قديم).
         *   لا شيءَ يفرضه، فمنحُه لا يعطي شيئاً ومنعُه لا يحمي شيئاً. يُسقَط،
         *   وتُنظَّف القاعدةُ من تلقاء نفسها عند أوّل حفظ.
         *
         * • **في الكتالوج لكنّه في `NEVER_FOR_EMPLOYEES`** ⇦ محاولةُ منحِ
         *   صلاحيةِ مسؤولٍ لموظف. تُرَدّ صراحةً وبصوتٍ عالٍ — هذه هي الحالةُ
         *   التي وُجد الفحصُ من أجلها.
         */
        $keys = [];
        foreach ($data['permissions'] as $key) {
            // ⚠ الممنوعُ أوّلاً: مفاتيحُ المسؤول ليست في الكتالوج، فلو سُئل
            // «هل هو معروف؟» قبلها لَسقطت كلُّ محاولةِ تصعيدٍ في خانة
            // «متقاعد» وخرجت صامتة.
            if (EmployeePermissions::forbiddenForEmployees($key)) {
                return $this->sendError('صلاحية لا تُمنح لموظف: ' . $key, [], 422);
            }
            if (!EmployeePermissions::exists($key)) {
                continue;                      // متقاعدٌ — يُسقَط بلا ضجيج
            }
            $keys[] = $key;
        }
        $data['permissions'] = array_values(array_unique($keys));
