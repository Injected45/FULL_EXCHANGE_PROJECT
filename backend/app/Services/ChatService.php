...
class ChatService
{
    ...
    public const MIMES = [
        ...
        //
        // ⚠ و`video/mp4` منها، وهي **الحالة الغالبة على أندرويد** لا حالةٌ
        // نادرة. مقيسٌ لا مفترَض: `finfo` يقرأ علامةَ `ftyp` في أول الملف —
        // فحاوية iOS تحمل `M4A ` وتُقرأ `audio/x-m4a`، بينما `MediaMuxer`
        // في أندرويد (وهي ما يستعمله `record` لصيغة AAC-LC) تكتب `mp42`
        // فتُقرأ **`video/mp4`**. فبغير هذا السطر تُسجَّل الرسالة الصوتية
        // على هاتف الموظف ثم يرفضها الخادم بـ«نوع غير مدعوم» — أي أنّ
        // الميزة تعمل على iOS وحدَه، وكلُّ أجهزة الفروع أندرويد.
        //
        // وثمنُه معروفٌ ومقبول: مقطعُ فيديو mp4 حقيقيّ — لو وصل — سيُصنَّف
        // صوتاً. ولا مسار في التطبيق يرفع فيديو أصلاً (صورٌ من `image_picker`
        // ورسائل صوتية و PDF)، وحدُّ الحجم قائمٌ فوق ذلك.
        'audio/mp4'   => ['AUDIO', 'm4a'],
