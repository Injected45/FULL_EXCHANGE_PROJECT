...
class EmployeePermissions
{
    ...
    public const CATALOG = [
        ...
        /*
         * ⚠ الحوالةُ الخارجية مفتاحٌ مستقلّ عن المحلية — لا امتدادٌ لها.
         *
         * أمرُ إعادة الهيكلة (10 سبتمبر 2026) جعل للموظف باباً إلى الحوالة
         * الخارجية طبقَ باب الوكيل. ولم يُلحق بـ`CREATE_TRANSFER` لسببين:
         *
         *   • **المخاطرة مختلفة**: الخارجيةُ تعبر حدوداً وعملات، وسعرُها
         *     يُحسب في محفّزٍ آخر بحدودٍ أخرى. ووكيلٌ يأذن لموظفه بحوالةٍ
         *     محلية لا يلزم أنه يأذن له بذلك.
         *   • **ولأن الإلحاق يمنح بأثرٍ رجعيّ**: كلُّ موظفٍ يملك
         *     `CREATE_TRANSFER` اليوم كان سيستيقظ غداً ومعه بابٌ لم يُمنحه
         *     أحد — وهو نقضُ قاعدة «لا شيءَ مفتوحٌ افتراضاً» من داخلها.
         *
         * ومفتاحٌ جديدٌ في الكتالوج يبدأ **ممنوعاً على الجميع** بلا ترحيلٍ
         * ولا إصدار: هذا هو معنى أن الصلاحيات صفوفٌ تُمنح لا أعمدةٌ تُقلب.
         */
        'CREATE_EXTERNAL_TRANSFER' => ['transfers', 'إنشاء حوالة خارجية'],
    ...
    public const LIVE = [
        ...
        // وكانت الوحيدةَ هنا التي تكتب في الدّفتر الماليّ.
        'CREATE_TRANSFER',

        /*
         * وإنشاءُ الحوالة الخارجية — أمرُ إعادة الهيكلة (10 سبتمبر 2026):
         * «تُحضَر من تطبيق الوكيل طبق الأصل».
         *
         * ⚠ الثانيةُ التي تكتب في الدّفتر، وبالقاعدة نفسِها: يُنادى
         * `transInsertExternal` بهويّة الوكيل عبر `EmployeeActsAsAgent`،
         * فلا منطقَ ماليَّ جديد ولا صفَّ يُميَّز عن صفّ الوكيل.
         */
