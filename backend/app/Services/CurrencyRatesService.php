...
class CurrencyRatesService
{
    ...
    public function list(): array
    {
        ...
        /*
         * ══════════════════════════════════════════════════════════════════
         *  ⚠⚠ لا يُعرض إلّا ما يُنفَّذ — أمرُ المالك (11 سبتمبر 2026)
         * ══════════════════════════════════════════════════════════════════
         *
         * «أيُّ عملةٍ بدون سعرٍ امنع ظهورَها في الشاشة … وإذا عملةٌ مسجّلةٌ
         *  خطأً وتسبّب خسائر فاجعلها في حكم غير المسجَّلة، ولا تظهر للعميل في
         *  الشاشة ولا في تنفيذ الحوالة الخارجية. والسعرُ الظاهر في الحوالة
         *  الخارجية يجب أن يكون ظاهراً في شاشة الأسعار».
         *
         * ⚠ والترشيحُ **بالحارس نفسِه** لا بشرطٍ مكتوبٍ هنا: شرطٌ ثانٍ يفترق
         * عن الحارس عند أوّل تعديل، فتُعرض خدمةٌ يرفضها الخادم أو تُخفى خدمةٌ
         * يقبلها. والأمرُ ينصّ على التطابق نصّاً.
         *
         * ⚠ وهذا **عكسُ** ما بُني قبل ساعة: كانت اللوحةُ تسرد غيرَ المسعَّر
         * بسببه كي يراه الوكيل. وأمرُ المالك رجّح الإخفاءَ — وهو أسلم: سطرٌ
         * أحمرُ على لوحةٍ يُقرأ خدمةً متاحةً عُطّلت مؤقّتاً، فيَعِد الوكيلُ
         * زبونَه بها. والغيابُ لا يُوعَد به.
         */
        $guard = app(ExternalPricingGuard::class);

        $rows = array_values(array_filter(
            $rows,
            fn ($r) => $guard->isAvailable((int) $r->country_id, (int) $r->service_id)
        ));
