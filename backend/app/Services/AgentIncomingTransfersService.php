...
class AgentIncomingTransfersService
{
    ...
    /**
     * حالةُ الاعتماد الوحيدة التي تصل بها الحوالةُ إلى الوكيل.
     *
     * ⚠ **المزامنةُ تشترطها عند الإدخال** (`ConfirmType = 2` حرفياً في
     * `syncFromCore`)، ولم يكن العرضُ ولا التسليمُ يُعيدان الشرط. فحوالةٌ
     * دخلت معتمدةً ثمّ **سُحب اعتمادُها في المنظومة** كانت تبقى في
     * «بانتظار التسليم» ويُسجَّل تسليمُها — أي يدفع الوكيل مالاً لحوالةٍ
     * لا تعترف بها المنظومة.
     *
     * وأمسكها فحصُ القبول: «غير المعتمدة لا تصل إلى الوكيل» (leaked=1).
     */
    public const CORE_APPROVED = 2;

    /**
     * الحالاتُ التي **يجوز** أن يراها الوكيل — قائمةُ سماحٍ لا منع.
     *
     * ⚠ 2 معتمدة، و3–6 ملغاة بعد اعتماد (يراها ليعلم أنها أُلغيت).
     * وما عداها — 0 و1 وأيُّ حالةٍ تُستحدث غداً — **لا تصل إليه**.
     *
     * وقائمةُ السماح مقصودة: قائمةُ المنع تُمرّر كلَّ حالةٍ جديدة تُضاف
     * إلى المنظومة، وهذا بابٌ يُفتح بلا أن يفتحه أحد.
     */
    public const CORE_VISIBLE = [2, 3, 4, 5, 6];

    /**
     * ⚠⚠ **البوّابة السيادية: لا تصل حوالةٌ إلى الوكيل قبل اعتمادها.**
     *
     * أمرُ المالك، وأعاده في 9 سبتمبر 2026 بعد أن وجد الشرطَ منفرطاً:
     * «عند تنفيذ حوالة من الرحالة لا تصل إلى الوكيل ولا يراها في التطبيق
     * ولا يصل إليه أيُّ إشعارٍ أو رسالة إلّا بعد أن تُعتمد من إدارة
     * الرحالة».
     *
     * ── لماذا انفرط ──────────────────────────────────────────────────
     *
     * `syncFromCore` كانت — وما زالت — لا تُدخل إلّا `ConfirmType = 2`.
     * فظُنّ الشرطُ محروساً عند الباب. لكنّ **الاعتماد يُسحب بعد الوصول**،
     * و`refreshCoreState` تُحدّث الرقم في الصفّ ولا تُخفيه. وكلُّ قارئٍ
     * كان يسأل «أليست ملغاة؟» — فتمرّ حالةُ عدم الاعتماد (0).
     *
     * وكان ذلك في **سبعة منافذ**: القائمة، والعدّادات، وجرس الإشعارات،
     * وتقرير المعلَّق للموظف، وملخّص الوكيل، وعرض حوالات الموظف، وشارةُ
     * «آخر العمليات» في الشاشة الرئيسية.
     *
     * ── ولماذا دالّةٌ واحدة ─────────────────────────────────────────
     *
     * ⚠ الشرطُ الذي يُكتب في سبعة مواضع يُنسى في الثامن. وهذه الدالّةُ
     * هي **الموضع الوحيد** الذي يعرف ما يُرى وما لا يُرى، ويحرسها
     * `tests/manual/approval_gate_check.php`: يفشل إن مسّ أحدٌ الجدولَ
     * بلا أن يمرّ بها.
     *
     * @param  \Illuminate\Database\Query\Builder  $q
     */
    public static function onlyApproved($q)
    {
        return $q
            /* ما محي أصلُه من المنظومة لا يُرى — انظر `reconcileMissing`. */
            ->whereNull('core_missing_at')
            /*
             * ⚠ و`null` مقبولةٌ عمداً: صفٌّ لم تُحدَّث حالتُه بعد. وهو لم
             * يدخل إلّا معتمداً — `syncFromCore` تشترط 2 — فإخفاؤه كان
             * يُفرغ الشاشةَ عند أوّل تشغيلٍ قبل أن تعمل المزامنة.
             */
            ->where(function ($w) {
                $w->whereNull('core_confirm_type')
                  ->orWhereIn('core_confirm_type', self::CORE_VISIBLE);
            });
    }
                         ...
                         int $perPage, ?int $deliveredByEmployeeId = null): array
    {
         ...
         * ولا ينتظر من يتذكّر أن يكتبها فيه.
         */
        $q = self::onlyApproved(
        ...
        if ($status === self::CANCELLED_TAB) {
             ...
             *
             * ⚠ الشرطُ كان «ليست ملغاة»، فكانت حالةُ **عدم الاعتماد** (0)
             * تمرّ: ليست في [3,4,5,6] فتُعرض على الوكيل مطالبةً بالتسليم.
             * والمزامنةُ لا تُدخل إلّا المعتمد، لكنّ الاعتمادَ يُسحب بعد
             * الإدخال — و`refreshCoreState` تُحدّث الرقم ولا تُخفي الصفّ.
             *
             * ⚠ و`null` مقبولةٌ عمداً: صفٌّ لم تُحدَّث حالتُه بعد. وإخفاؤه
             * كان سيُفرِغ التبويبَ عند أوّل تشغيلٍ قبل أن تعمل المزامنة.
             */
            $q->where('status', self::PENDING)
              ->where(function ($w) {
                  $w->whereNull('core_confirm_type')
                    ->orWhere('core_confirm_type', self::CORE_APPROVED);
              });   // والملغاةُ تخرج من هنا وتبقى في تبويبها
    ...
    public function markDelivered(int $agentId, int $id, int $userId, array $trace): array
    {
        return DB::transaction(function () use ($agentId, $id, $userId, $trace) {
            ...
            /*
             * ⚠ **وغيرُ المعتمدة لا تُسجَّل تسليماً** — الحارسُ الثالث.
             *
             * إخفاؤها من التبويب تجميل: الشاشةُ قد تكون مفتوحةً منذ ما قبل
             * سحب الاعتماد، والطلبُ قد يأتي من خارج التطبيق. وتسجيلُ تسليمٍ
             * لحوالةٍ لا تعترف بها المنظومة يجعل الدفترَ يشهد بما لم يقع.
             *
             * و`null` تمرّ كما في العرض: صفٌّ لم تُحدَّث حالتُه بعد.
             */
            if ($row->core_confirm_type !== null
                && (int) $row->core_confirm_type !== self::CORE_APPROVED) {
                return [
                    'changed'      => false,
                    'row'          => $row,
                    'not_approved' => true,
            ...
            DB::table('transfer_status_history')->insert([
                ...
                /*
                 * ⚠ **صفُّ التحوُّل يقول من حوّله — لا جدولٌ آخر.**
                 *
                 * `changed_by` يحمل معرّفَ الوكيل في المسارين، لأن الموظف
                 * ينفّذ بوصفه واجهةً من وكيله. فبلا هذا العمود لا يُفرَّق
                 * تسليمُ الوكيل من تسليم موظّفه إلّا بالتوفيق بين ثلاثة
                 * جداول بالتاريخ — ومن يقرأ تاريخ الحوالة يقرأ هذا الجدول.
                 *
                 * `null` في مسار الوكيل، وهو الصادق: لم يُنفّذها موظف.
                 */
                'changed_by_employee_id' => $trace['employee_id'] ?? null,
