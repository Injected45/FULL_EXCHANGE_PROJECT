/* ============================================================================
   سكربت نشر — مركز الدعم، الدفعة الأولى من التطوير التشغيلي
   التاريخ: 2026-09-07
   (بنود المالك 3 أولوية · 4 تصنيف · 7 ملاحظات داخلية · 12 مرجع · 17 شريط)
   ============================================================================

   ⚠ **لا أثر ماليّ.** صفرُ جداولَ مالية، وصفرُ مفاتيحَ إليها، وصفرُ
   محفّزات. المفاتيح كلُّها إلى `chat_threads` و`support_tag_defs`.

   ⚠ **تطويرٌ فوق القائم لا بناءٌ جديد:** `support_thread_state` تُوسَّع
   بأعمدة ولا تُستبدل، والملاحظةُ الداخلية عمودٌ على `chat_messages` لا
   جدولُ رسائلَ ثانٍ.

   ── ما يُضاف ──
   | الجدول/العمود | لماذا |
   |---|---|
   | أعمدة على `support_thread_state` | أولوية · تصنيف · مرجع · تأجيل · أزمنة SLA |
   | `support_categories` | تصنيفاتٌ تديرها الإدارة بلا تعديل شيفرة |
   | `support_tag_defs` + `support_thread_tags` | وسومٌ كثيرٌ لكثير |
   | `support_events` | الشريط الزمني — Metadata تشغيلية فقط |
   | `chat_messages.is_internal` | الملاحظة الداخلية |

   ── التشغيل ──
     sqlcmd -S <SERVER> -d EXCHANGESYS2026 -i 2026-09-07_support_ops.sql

   خاملٌ: تشغيلُه مرّتين لا يضرّ (مُجرَّب: 16 دفعة، الإعادةُ بلا أثر).

   ⚠ ويُشترط أن يكون سكربت 2026-09-06_support_center.sql قد شُغِّل قبله.
   ============================================================================ */
...
/* ============================================================================
   تحقّقٌ بعد التشغيل
   ============================================================================ */
SELECT name AS [جدولٌ جديد],
       (SELECT COUNT(*) FROM sys.columns WHERE object_id = t.object_id) AS [أعمدة]
  FROM sys.tables t
 WHERE name IN ('support_categories', 'support_tag_defs',
                'support_thread_tags', 'support_events')
 ORDER BY name;
GO

SELECT CASE WHEN COL_LENGTH('dbo.chat_messages','is_internal') IS NULL
            THEN N'✗ غائب' ELSE N'✓ موجود' END AS [chat_messages.is_internal],
       CASE WHEN COL_LENGTH('dbo.support_thread_state','priority') IS NULL
            THEN N'✗ غائب' ELSE N'✓ موجود' END AS [support_thread_state.priority],
       CASE WHEN COL_LENGTH('dbo.support_thread_state','reference') IS NULL
            THEN N'✗ غائب' ELSE N'✓ موجود' END AS [support_thread_state.reference];
GO

/* التصنيفاتُ المزروعة — عشرةٌ وخمسةُ وسوم. */
SELECT (SELECT COUNT(*) FROM dbo.support_categories) AS [تصنيفات],
       (SELECT COUNT(*) FROM dbo.support_tag_defs)   AS [وسوم];
GO
