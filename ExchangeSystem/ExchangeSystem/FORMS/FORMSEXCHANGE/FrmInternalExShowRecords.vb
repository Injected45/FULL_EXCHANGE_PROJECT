...
Public Class FrmInternalExShowRecords
    Dim DT As New DataTable
    Sub LoadData()
        Dim PRM(0) As SqlParameter
        PRM(0) = New SqlParameter("@RecievedBranchID", SqlDbType.Int)
        PRM(0).Value = BID
        DT.Clear()
        DT = RUN_QUARY_PRO("InternalExValues_ShowRecords", PRM)
    ...
    Private Sub GVRole_DoubleClick(sender As Object, e As EventArgs) Handles GVROLE.DoubleClick
        If GVROLE.RowCount > 0 Then
            ...
            If info.InRow OrElse info.InRowCell Then
                ...
                FRMINTERNALTRANSFER.LOADBRNACH()
                'FRMINTERNALTRANSFER.LOADDELIVERYBRANCH()
                'FRMINTERNALTRANSFER.LOADBRANCHSERVICES()
                FRMINTERNALTRANSFER.BtnSave.Enabled = False
                FRMINTERNALTRANSFER.BtnEdit.Enabled = True
                FRMINTERNALTRANSFER.BtnDelete.Enabled = True
                FRMINTERNALTRANSFER.BtnPrint.Enabled = True
                FRMINTERNALTRANSFER.ShowCurrentRecord(CO)
                Me.Close()
            End If
        End If
