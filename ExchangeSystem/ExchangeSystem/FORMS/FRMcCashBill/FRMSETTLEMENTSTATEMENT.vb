...
Public Class FRMSETTLEMENTSTATEMENT
    Sub LOADDATA()
        ...
        PR(3) = New SqlParameter("@IsSettlement", SqlDbType.Int) With {.Value = IsSettlement.SelectedIndex}
        Dim DT As New DataTable
        DT.Clear()
        DT = RUN_QUARY_PRO("AccSafeActivityTb_GETPCEMPID", PR)
        If DT.Rows.Count > 0 Then
            GCRole.DataSource = DT
            GVRole.Columns("طبيعة الحركة").Width = 400
            GVRole.Columns("ملاحظات").Width = 400
            DVGFROMAT()
        End If
    End Sub
    ...
    Private Sub FRMSETTLEMENTSTATEMENT_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        DVGFROMAT()
        LOADBRANCH()
        IsSettlement.SelectedIndex = -1
    ...
    Private Sub IsSettlement_TextChanged(sender As Object, e As EventArgs) Handles IsSettlement.TextChanged
        BranchID.EditValue = -1
        GVRole.Columns.Clear()
        GCRole.DataSource = Nothing
    End Sub
    ...
    Private Sub GCRole_Click(sender As Object, e As EventArgs) Handles GCRole.Click
        If IsSettlement.SelectedIndex = 1 Then
            FRMSETTLEMENTSTATEMENTDETAILS.GCRole.DataSource = Nothing
            FRMSETTLEMENTSTATEMENTDETAILS.GVRole.Columns.Clear()
            FRMSETTLEMENTSTATEMENTDETAILS.ShowDialog()
        End If
    End Sub
    ...
    Private Sub SimpleButton1_Click(sender As Object, e As EventArgs) Handles SimpleButton1.Click
        ...
        Try
            ...
            PRM(3) = New SqlParameter("@IsSettlement", IsSettlement.SelectedIndex)
            Dim dt As DataTable = RUN_QUARY_PRO("ZRPT_AccSafeActivityTb_GETPCEMPID", PRM)
            dt.TableName = "AccountsTb"
            Dim ds As New DataSet
            ds.Tables.Add(dt)
            If dt.Rows.Count > 0 Then
                Dim report As New RPTSETTLEMENTSTATEMENT
