    ...
    Private Sub GVRole_Click(sender As Object, e As EventArgs) Handles GVRole.Click
        ...
        If Application.OpenForms().OfType(Of FrmProPettyCash).Any Then
            If info.InRow OrElse info.InRowCell Then
                ...
                FrmProPettyCash.BtnEdit.Caption = "استرجاع القيمة"
            End If
        Else
            If info.InRow OrElse info.InRowCell Then

                Dim CO As String = view.GetFocusedRowCellValue("الرمز").ToString
                FRMPettyCash.IsUpdate = True
                FRMPettyCash.SHOW_EMCUSCODE(CO)
                FRMPettyCash.BtnSave.Enabled = False
                'FRMPettyCash.BtnPrint.Visibility = DevExpress.XtraBars.BarItemVisibility.Never
                FRMPettyCash.BtnDelete.Visibility = DevExpress.XtraBars.BarItemVisibility.Never
                FRMPettyCash.BtnEdit.Enabled = True
                FRMPettyCash.BtnPrint.Enabled = True
                FRMPettyCash.DISAPLEDCONTROLS()
                FRMPettyCash.BtnEdit.Caption = "استرجاع القيمة"
            End If
        End If
        Me.Close()
    End Sub
