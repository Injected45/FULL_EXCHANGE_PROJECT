...
Public Class FrmAddProject
    ...
    End Sub
    Sub newRecord()
        Code.Text = GETMAXID("ContractDB.dbo.ProjectTb", "ID") + 1
        ProName.Text = ""
        ProName.Focus()
        LOADBRANCH()
        BranchID.EditValue = -1
        Projectvalue.EditValue = 0.000
        IsActive.IsOn = True
        IsActive.IsOn = True
    ...
    Public Sub CUSTOMER_INSERT()
        ...
        PRM(1) = New SqlParameter("@ProName", SqlDbType.NVarChar, -1) With {.Value = ProName.Text.Trim}
        ...
        PRM(8) = New SqlParameter("@Projectvalue", SqlDbType.Decimal) With {.Value = Projectvalue.EditValue}
        RUN_EXUTE_PRO("CONDB_ProjectTb_Insert", PRM)
    ...
    Public Sub Pro_SelectByID(x)
        ...
        Dim dt As New DataTable
        dt.Clear()
        dt = RUN_QUARY_PRO("CONDB_ProjectTb_Select", PRM)
        If dt.Rows.Count > 0 Then
            Code.Text = dt.Rows(0).Item("ID")
            ProName.Text = dt.Rows(0).Item("ProName")
            BranchID.EditValue = dt.Rows(0).Item("BranchID")
            IsActive.IsOn = dt.Rows(0).Item("IsActive")
            Projectvalue.EditValue = dt.Rows(0).Item("Projectvalue")
        End If
    End Sub
    Public Sub DISAPLEDCONTROLS(IsEnabled As Boolean)
        ...
        'Projectvalue.Enabled = IsEnabled
    End Sub
