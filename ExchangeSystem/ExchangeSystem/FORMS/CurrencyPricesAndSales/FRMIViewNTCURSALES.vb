...
Public Class FRMIViewNTCURSALES
    Public IsBuyORSale As Byte
    Sub LOADDATA()
        GCRole.DataSource = Nothing
        GVRole.Columns.Clear()
        Dim PRM(0) As SqlParameter
        PRM(0) = New SqlParameter("@UserInsert", SqlDbType.Int) With {.Value = UserID}
        Dim PR(0) As SqlParameter
        PR(0) = New SqlParameter("@UserInsert", SqlDbType.Int) With {.Value = UserID}
        Dim DT As New DataTable
        DT.Clear()
        DT = RUN_QUARY_PRO("CurrenciesBuyandsellTB_SelectAll", PRM)
        Dim DT1 As New DataTable
        DT1.Clear()
        DT1 = RUN_QUARY_PRO("CurrenciesBuyandsellTB_SelectAll2", PR)
    ...
    Sub DVGFROMAT()
        ...
        'GVRole.Columns("لحساب").Width = 350
        'GVRole.Columns("من حساب").Width = 350
    ...
    Private Sub GVRole_Click(sender As Object, e As EventArgs) Handles GVRole.DoubleClick
        ...
        If info.InRow OrElse info.InRowCell Then
            ...
            If IsBuyORSale = 1 Then
                FRMINTCURSALES.GetRecord(CO)
            Else
                FRMBuycurrencyfromacustomer.GetRecord(CO)
            End If
