...
Public Class FROWNMCURRENCYPRICE
    Dim clscp As New CLSOWNCURRENCYPRICE
    ...
    Public Sub lodePreportes()
        ...
        dt.Clear()
        dt = SElectUEserFormButtn(15, UserID)
        If dt.Rows.Count > 0 Then
            ...
            If dt.Rows(0)("CanDelete") = 0 Then BtnPrint.Visibility = DevExpress.XtraBars.BarItemVisibility.Never Else BtnPrint.Visibility = DevExpress.XtraBars.BarItemVisibility.Always
        End If

    End Sub
    Sub NEWRECORD()
        LOADCIDFROM()
        CodeID.Text = GETMAXID("CurrencyPricesOwnTb", "ID") + 1
    ...
    Public Overrides Sub SetData()
        ...
        If GVRole.RowCount = 0 Then
            ErrorMessage(Me, "يجب اختيار سعر واحد على الأقل", "رسالة خطأ")
            Exit Sub
        End If
        If TypeID.SelectedIndex = 0 Then
            IsAgent = False
        ...
        Else
            IsAgent = True
            AgentID = BranchID.EditValue
        End If

        clscp.CURRENCYPRICE_Insert(CodeID.Text, CPDInsert, CurrencyPower.SelectedIndex, IsUpdate, IsAgent, AgentID)
        MyBase.SetData()
    End Sub
...
#End Region
    Private Sub SalePrice_Leave(sender As Object, e As EventArgs) Handles SPrice.Leave
        ...
        'Dim firn As Decimal = 1.0
        'Dim secon As Decimal = 9.352
        'secon = 9.352
        'Dim resu As Decimal = firn / secon
        'BPrice.EditValue = resu
        'Dim DecimalLength = secon * resu
    ...
    Private Sub TypeID_TextChanged(sender As Object, e As EventArgs) Handles TypeID.TextChanged
        If TypeID.SelectedIndex = 0 Then
            BranchID.Enabled = False
        Else
            BranchID.Enabled = True
    ...
    End Sub
End Class
Public Class CLSOWNCURRENCYPRICE
    Public Sub CURRENCYPRICE_Insert(ID As ULong, dt As DataTable, CurrencyPower As Boolean, IsUpdate As Boolean, IsAgent As Boolean, AgentID As Integer)
        ...
        PRM(4) = New SqlParameter("@IsAgent", SqlDbType.Bit) With {.Value = IsAgent}
        PRM(5) = New SqlParameter("@AgentID", SqlDbType.Int) With {.Value = AgentID}
        RUN_EXUTE_PRO("CurrencyPricesOwnTb_Insert", PRM)
